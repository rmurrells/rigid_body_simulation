use crate::{
    math::{
        matrix::Matrix3x3,
        matrix_vector, moment_of_inertia,
        polyhedron::{Edge, Polyhedron},
        vector::Vector3d,
        Quarternion,
    },
    mesh::Mesh,
...
impl RigidBody {
    ...
    pub fn cuboid(
        ...
        Self::new(
            mass_inv,
            &if mass_inv >= EPSILON {
                moment_of_inertia::aligned_cuboid(dimensions, 1. / mass_inv)
                    .inverse()
                    .expect("cuboid - inertia_body inverse")
            } else {
                Matrix3x3::default()
            },
    ...
    pub fn from_mesh(
        ...
        Ok(Self::new(
            mass_inv,
            inertia_body_inv,
            Polyhedron::new(face_vertex_indices, edges, mesh.vertices.clone())?,
            position,
            rotation,
            momentum,
            angular_momentum,
    ...
    pub fn ki_translational(&self) -> f64 {
        if self.is_immovable() {
            ...
            0.5 * self.velocity.mag_sq() / self.mass_inv
        }
    }
    ...
    pub fn update_geometry(&mut self) {
        self.bounding_box =
            [Vector3d::new(MAX, MAX, MAX), Vector3d::new(MIN, MIN, MIN)];
        ...
        for i in 0..vertices_world.len() {
            ...
            *vertex = matrix_vector::mult_3(&self.rotation, &vertices_body[i])
                .add(&self.position);
            for j in 0..3 {
    ...
    fn update_rotation(&mut self) {
        ...
        self.inertia = self
            .rotation
            .mult(&self.inertia_body)
            .mult_t(&self.rotation);
        self.inertia_inv = self
            .rotation
            .mult(&self.inertia_body_inv)
            .mult_t(&self.rotation);
    }
}
