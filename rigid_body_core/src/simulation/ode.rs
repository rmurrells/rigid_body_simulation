...
pub fn euler(delta_t: f64, rigid_bodies: &mut [RigidBody]) {
	...
	rigid_body.position.add_assign(
	    &rigid_body.velocity().scale(delta_t),
	);
	rigid_body.momentum.add_assign(&rigid_body.force.scale(delta_t));
	...
	rigid_body.angular_momentum.add_assign(&rigid_body.torque.scale(delta_t));
	rigid_body.update();
    }
}
