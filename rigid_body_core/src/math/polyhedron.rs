use super::{geometry, vector::Vector3d};
...
impl Face {
    fn new(
        ...
        let (connected_edge_indices, edge_indices) =
            Self::get_edges(&vertex_indices, edges);
        let mut ret = Self {
            direction: Vector3d::default(),
    ...
    fn update(&mut self, edges: &[Edge]) {
        self.direction = edges[self.edge_indices[0]]
            .direction
            .cross(&edges[self.edge_indices[1]].direction)
            .normal();
        if self.flip_direction {
            self.direction.scale_assign(-1.);
        }
    }
}
