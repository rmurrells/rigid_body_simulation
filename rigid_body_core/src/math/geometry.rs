...
pub fn plane_line_intersection(
    ...
    if dp.abs() < EPSILON {return None;}
    Some(line.pos.add(&line.dir.scale(
	plane.pos.sub(&line.pos).dot(&plane.dir)/dp,
    )))
}
...
pub fn raw_finite_line_closest_dist_sq(
    start_1: &Vector3d, end_1: &Vector3d,
    start_2: &Vector3d, end_2: &Vector3d,
	...
	if mag_sq_2 <= 0. {
		...
		l1 = clamp((dp_12*dp_2_sd-dp_1_sd*mag_sq_2)/den, 0., 1.);
	    }
...
pub fn raw_plane_finite_line_intersection(
    ...
    if d < 0. || d > 1. {None}
    else {Some(pos_1.add(&line_dir.scale(d)))}
}
