...
use crate::{math::vector::Vector3d, render::Camera};
...
impl CameraMover {
    ...
    fn move_rel(&self, mouse_state: &MouseState, camera: &mut Camera) {
        ...
        camera.position = self.center.sub(&camera.dir().scale(dist));
    }
    ...
    fn move_fps(&self, input: &InputCore, camera: &mut Camera) {
        ...
        let move_lin = (input.keyboard_state.get(Keycode::W) as i32
            - input.keyboard_state.get(Keycode::S) as i32)
            as f64
            * self.move_fact;
        let move_hor = (input.keyboard_state.get(Keycode::D) as i32
            - input.keyboard_state.get(Keycode::A) as i32)
            as f64
            * self.move_fact;
        let move_vert = (input.keyboard_state.get(Keycode::Q) as i32
            - input.keyboard_state.get(Keycode::E) as i32)
            as f64
            * self.move_fact;
