...
use crate::{
    math::{
        geometry::{self, FiniteLine3d, Plane},
        matrix::{Matrix3x3, Matrix4x4},
        matrix_vector,
        triangle::{Triangle3d, Triangle4d},
        vector::{Vector3d, Vector4d},
    },
    mesh::{Mesh, MeshTriangle},
};
use std::f64::EPSILON;
...
impl RenderObjectCreator {
    pub fn new(window_size: (u32, u32), near: f64, far: f64, fov: f64) -> Self {
        let window_hsize =
            (f64::from(window_size.0) / 2., f64::from(window_size.1) / 2.);
        Self {
            window_hsize,
            near,
            far,
    ...
    pub fn get_window_line(
        ...
        let mut ret =
            RenderLine::from_vertices(&camera.view(start), &camera.view(end));
    ...
    pub fn get_window_triangles(
        ...
        for mesh_triangle in &mesh.mesh_triangles {
            ...
            if world_normal
                .dot(&camera.position.sub(&world_triangle.vertices[0]))
                < 0.
            {
                continue;
            }
            let camera_triangle =
                Self::camera_triangle(camera, &world_triangle);
            ...
            match self.clip_triangle_on_plane(
                &RenderTriangle::new(&camera_triangle, light_value),
                &self.near_plane,
            ) {
                ClipTriangleOnPlane::Inside => add_render_triangle(
                    &RenderTriangle::new(&camera_triangle, light_value),
                ),
                ClipTriangleOnPlane::PartOne(new) => add_render_triangle(&new),
    ...
    fn camera_triangle(camera: &Camera, triangle: &Triangle3d) -> Triangle3d {
        Triangle3d::new(
    ...
    fn clip_line_on_plane(
        ...
        let intercept = Self::get_intercept(plane, &line.start, &line.end);
        if ds >= 0. {
            line.end = intercept;
        } else {
    ...
    fn clip_pos_on_plane(plane: &Plane, pos: &Vector3d) -> Option<()> {
        if geometry::pos_plane_signed_dist(pos, plane) < 0. {
            return None;
        }
        Some(())
    }

    fn clip_triangles(&self, triangles: &mut Vec<RenderTriangle>) {
    ...
    fn clip_triangle_on_plane(
        ...
        match inside_count {
            ...
            1 => {
                ...
                ClipTriangleOnPlane::PartOne(RenderTriangle::from_vertices(
                    inside_vert,
                    &intercept_1,
                    &intercept_2,
            ...
            2 => {
                ...
                let intercept_1 =
                    Self::get_intercept(plane, outside_vert, inside_vert_1);
                let intercept_2 =
                    Self::get_intercept(plane, outside_vert, inside_vert_2);
    ...
    fn dehomogenized_pos(pos: &Vector4d) -> Vector3d {
        ...
        Vector3d::new(pos[0] / div, pos[1] / div, pos[2] / div)
    }
    ...
    fn get_intercept(
        ...
        if let Some(intercept) =
            &geometry::plane_finite_line_intersection(plane, inside, outside)
        {
            *intercept
        } else {
            *outside
    ...
    fn map_triangles_to_window(&self, triangles: &mut [RenderTriangle]) {
    ...
    fn map_pos_to_window(&self, pos: &mut Vector3d) {
...
impl RenderLine {
    fn from_vertices(vertex_1: &Vector3d, vertex_2: &Vector3d) -> Self {
        Self {
...
impl RenderTriangle {
    fn new(triangle_3d: &Triangle3d, light_value: f64) -> Self {
        Self {
    ...
    fn from_vertices(
        ...
        Self {
            triangle_3d: Triangle3d::new(vertex_1, vertex_2, vertex_3),
            light_value,
        }
    }
}
